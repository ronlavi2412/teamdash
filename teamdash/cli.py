...
def _add_fetch_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--no-scoring",
        action="store_true",
        help="Skip complexity point estimation (faster, fewer API calls)",
    )
    parser.add_argument(
        "--refresh-gitlab",
        action="store_true",
...
def _do_fetch(args: argparse.Namespace) -> tuple:
    ...
    if not args.refresh_gitlab and config.github_orgs and not check_auth():
        print(
            "[ERROR] GitHub CLI not authenticated. Run: gh auth login", file=sys.stderr
        )
        sys.exit(1)
    ...
    print(
        f"Fetching data for {config.team_name} ({len(config.engineers)} engineers, "
...
def _cmd_fetch(args: argparse.Namespace) -> None:
    ...
    data = build_dashboard_data(
        config, summaries, jira_raw=jira_raw, cycle_time_data=cycle_time_data
    )
...
def _cmd_fetch_jira(args: argparse.Namespace) -> None:
    ...
    if not check_jira_auth(config.jira.cloud_id, email, token):
        print(
            "[ERROR] Jira authentication failed. Check JIRA_EMAIL and JIRA_API_TOKEN",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    quarters = get_quarters(args.quarters, include_current=args.include_current)
    print(
        f"Fetching Jira data for {config.team_name} ({len(quarters)} quarters)...",
        file=sys.stderr,
    )
...
def main() -> None:
    ...
    if args and args[0] == "fetch":
        ...
        cycle_time_data = jira_raw.get("cycle_times") if jira_raw else None
        generate_dashboard(
            config, summaries, parsed.output, cycle_time_data=cycle_time_data
        )
